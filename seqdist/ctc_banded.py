# AUTOGENERATED! DO NOT EDIT! File to edit: notebooks/02_CTC_loss_banded.ipynb (unless otherwise specified).
...
def loss_masked(logits, targets, input_lengths, target_lengths, base_alignments, width):
    ...
    state_scores = torch.where((window_starts <= states) & (states < window_starts + width), state_scores, logits.new_full((1,), ctc.Log.zero))
...
def _logz_fwd(state_scores, repeat_mask, final_states, input_lengths, window_starts, S:ctc.semiring=ctc.Log):
...
# Cell
def fwd_bwd(state_scores, repeat_mask, final_states, input_lengths, window_starts, fwd_bwd_impl, S:ctc.semiring=ctc.Log):
    T, N, W = state_scores.shape
...
def _fwd_bwd_py(alpha, beta, state_scores, repeat_mask, input_lengths, window_starts, S:ctc.semiring=ctc.Log):
    T, N, W = state_scores.shape
    zeros = state_scores.new_full((N, W + 2), S.zero)
    range_W = torch.arange(W + 2, device=state_scores.device)
    #fwd
...
class _Logz(torch.autograd.Function):
    ...
    def forward(ctx, state_scores, repeat_mask, final_states, input_lengths, window_starts, fwd_bwd_impl):
        alpha, beta, logz = fwd_bwd(state_scores, repeat_mask, final_states, input_lengths, window_starts, fwd_bwd_impl, ctc.Log)
...
class _LogzViterbi(torch.autograd.Function):
    @staticmethod
    def forward(ctx, state_scores, repeat_mask, final_states, input_lengths, window_starts, fwd_bwd_impl):
        alpha, beta, logz = fwd_bwd(state_scores, repeat_mask, final_states, input_lengths, window_starts, fwd_bwd_impl, ctc.Max)
        ctx.save_for_backward(alpha, beta, input_lengths)
        return logz
    ...
    def backward(ctx, grad):
        alpha, beta, input_lengths = ctx.saved_tensors
        g = ctc.max_grad(alpha[1:] + beta[1:], dim=2) * ctc.masked_grad(grad.expand(alpha.size(0) - 1, -1), input_lengths)
        return g, None, None, None, None, None
